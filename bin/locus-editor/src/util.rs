...
pub(crate) fn color_to_egui(c: locus_geom::Color) -> egui::Color32 {
    let [r, g, b, a] = c.to_srgb8();
    egui::Color32::from_rgba_unmultiplied(r, g, b, a)
}

/// Convert egui's sRGB `Color32` to our linear RGBA `Color`.
pub(crate) fn egui_to_color(c: egui::Color32) -> locus_geom::Color {
    locus_geom::Color::from_srgb8(c.r(), c.g(), c.b(), c.a())
}
...
pub(crate) fn combined_bounds(scene: &Scene, node_ids: &[NodeId]) -> locus_geom::Bounds {
    locus_bool::selection_visual_bounds(scene, node_ids)
}
...
pub(crate) fn node_display(node: &locus_scene::Node) -> (&'static str, String) {
