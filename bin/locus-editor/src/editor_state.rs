...
impl EditorState {
    ...
    pub(crate) fn insert_raster_from_bytes(
        ...
        let mut node = locus_scene::Node::raster(label, image, w, h);
    ...
    pub(crate) fn load_svg_replace(
        ...
        let scene = locus_svg::import_svg(bytes).map_err(|e| format!("{e}"))?;
        self.scene = scene;
    ...
    pub(crate) fn load_svg_as_group(
        ...
        let source = locus_svg::import_svg(bytes).map_err(|e| format!("{e}"))?;
        let label = path
            .file_stem()
            .and_then(|s| s.to_str())
        ...
        let new_group =
            locus_scene::merge_as_group(&mut self.scene, dest_parent, &source, label)
    ...
    pub(crate) fn snapshot_gradient_for_drag(&mut self) {
        ...
        if let Some(node) = self.scene.get(handle.paint)
            && let NodeData::Paint(locus_scene::Paint::Gradient(ref g)) = node.data
        {
    ...
    pub(crate) fn record_gradient_drag_undo(&mut self) {
        ...
        if !matches!(
            ...
            Some(NodeData::Paint(locus_scene::Paint::Gradient(_)))
        ) {
            return;
        }
        self.history
    ...
    pub(crate) fn nudge_selection(&mut self, dx: f64, dy: f64) -> bool {
        ...
        if !self.select_state.selected.is_empty() {
            ...
            for vr in self.select_state.selected.clone() {
                ...
                if matches!(
                    vr.kind,
                    locus_tools::PointKind::CubicCtrl1
                        | locus_tools::PointKind::CubicCtrl2
                        | locus_tools::PointKind::QuadCtrl
                ) {
                    locus_tools::enforce_vertex_constraint(&vr, &mut self.scene);
                }
                moved = true;
            }
            if moved {
    ...
    pub(crate) fn try_delete_focused_gradient_stop(&mut self) -> bool {
        ...
        let used_by_selection = self.select_state.selected_nodes.iter().any(|&nid| {
            ...
            node.for_each_paint_ref(|p| {
                if matches!(p, locus_scene::PaintRef::Ref(id) if *id == paint_id) {
                    found = true;
                }
            });
            found
        ...
        let locus_scene::NodeData::Paint(locus_scene::Paint::Gradient(g)) = &node.data else {
            self.gradient_stop_focus = None;
            return false;
        };
        if idx >= g.stops.len() {
    ...
    pub(crate) fn delete_with_undo(&mut self) -> bool {
        if !self.select_state.selected.is_empty() {
            ...
            struct NodeInfo {
                ...
                parent: NodeId,
                index: usize,
                snapshot: locus_scene::NodeSnapshot,
            }
