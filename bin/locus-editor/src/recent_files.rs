...
//! (`~/.config/locus-editor/recent.json` on Linux, equivalent on macOS /
...
fn config_path() -> Option<PathBuf> {
    ...
    // result: `~/.config/locus-editor` on Linux, sensible defaults on
    // macOS / Windows. The `application` arg drives the bundle name.
    directories::ProjectDirs::from("", "", "locus-editor").map(|d| d.config_dir().join(CONFIG_FILE))
}
