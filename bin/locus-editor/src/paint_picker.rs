...
//!
//! All colour values cross the boundary as `locus_geom::Color` (linear
...
use locus_scene::{NodeData, Paint, PaintRef, Scene};
...
mod tests {
    ...
    fn document_palette_dedups_and_orders_by_usage() {
        use locus_scene::style::{Fill, FillRule};
        use locus_scene::{
        ...
        for fill in [&red_fill, &red_fill, &blue_fill] {
            let mut node = Node::path("p", locus_geom::Path::new());
        ...
        let grad = Gradient {
            kind: GradientKind::Linear {
                start: locus_geom::Point::new(0.0, 0.0),
                end: locus_geom::Point::new(1.0, 0.0),
            },
            stops: vec![ColorStop {
                offset: 0.0,
            ...
            transform: locus_geom::Affine::IDENTITY,
