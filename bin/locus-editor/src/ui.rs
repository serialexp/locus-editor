...
use locus_tools::{PenAction, SelectState, TextAction, ToolType};
...
fn load_svg_from_path(
    ...
    match std::fs::read(path) {
        Ok(data) => match locus_svg::import_svg(&data) {
            Ok(new_scene) => {
                *scene = new_scene;
                *history = History::new();
...
pub(crate) fn run_ui(
    ...
    let menu_resp = egui::Panel::top("menu_bar").show(ctx, |ui| {
        egui::MenuBar::new().ui(ui, |ui| {
            ...
            ui.menu_button("Edit", |ui| {
                ...
                // to fit.
                let has_content = !locus_bool::scene_content_bounds(scene).is_empty();
                if ui
            ...
            ui.menu_button("Path", |ui| {
                    ...
                    .clicked()
                {
                    structure_cmds.push(StructureAction::MakeBooleanGroup {
                        nodes: selection.selected_nodes.clone(),
                        op: locus_scene::BoolOp::Difference,
                    });
                    ui.close();
                }
                if ui
                    ...
                    .clicked()
                {
                    structure_cmds.push(StructureAction::MakeBooleanGroup {
                        nodes: selection.selected_nodes.clone(),
                        op: locus_scene::BoolOp::Intersect,
                    });
                    ui.close();
                }
                if ui
                ...
                let flatten_target = selection.selected_nodes.iter().copied().find(|&id| {
                    matches!(
                        ...
                        Some(locus_scene::NodeData::Group {
    ...
    if fit_page_to_content_requested {
        ...
        let content = locus_bool::scene_content_bounds(scene);
        if !content.is_empty() {
            let margin = 1.0;
