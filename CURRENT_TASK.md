...
- **SVG import** via usvg (solid colors, linear/radial gradients, paths, groups, transforms)
- **SVG export**: all 4 segment types (L/Q/C/A), fill/stroke, transforms, viewBox, gradient defs with url(#id) references
- **Gradient rendering**: fragment shader evaluation (pixel-perfect), linear + radial (with focal offset), pad/reflect/repeat spread methods, gradient transforms, up to 8 color stops per gradient
...
- **World transforms**: applied throughout rendering, hit testing, handles, bounds, vertex dragging
- **Selection bounding boxes**: outline around selected objects/groups
...
1. **Text rendering** — vector-text is a stub (rustybuzz + ttf-parser deps wired but no implementation)
2. **Pattern rendering** — patterns not yet supported
3. **Stroke dash patterns** — not rendered yet
4. **OKLab gradient interpolation** — InterpolationSpace::OkLab defined but shader only does LinearRgb
