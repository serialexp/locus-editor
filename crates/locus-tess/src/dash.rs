    ...
    for seg in &subpath.segments {
        ...
        if seg_len < 1e-12 {
            ...
            if let Some(ref mut dash) = current_dash {
                dash.push_segment(*seg, locus_geom::VertexMode::Corner);
            }
        ...
        while seg_distance_consumed < seg_len - 1e-12 {
            ...
            if remaining_in_element >= seg_remaining_len - 1e-12 {
                ...
                if let Some(ref mut dash) = current_dash {
                    dash.push_segment(remaining_seg, locus_geom::VertexMode::Corner);
                }
            ...
            if let Some(ref mut dash) = current_dash {
                dash.push_segment(first, locus_geom::VertexMode::Corner);
            }
