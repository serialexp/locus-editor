...
use vector_scene::{Node, NodeData, NodeId, Scene, TextData};
...
impl TextToolState {
    ...
    pub fn cursor_local_position(&self, scene: &Scene) -> Option<(f64, f64, f64)> {
        ...
        let positions =
            vector_text::cursor_positions(&text.content, &text.font_family, text.font_size);
        let shaped = vector_text::shape_text(&text.content, &text.font_family, text.font_size);
