//! Polygon boolean operations on `locus_geom::Path`, for non-destructive
...
use locus_geom::{Affine, Bounds, Path, Point, Segment, SubPath, VertexMode};
use locus_scene::{BoolOp, GroupKind, NodeData, NodeId, Scene};
...
pub fn boolean_group_visual_bounds(scene: &Scene, group_id: NodeId, world: Affine) -> Bounds {
    ...
    let local = locus_tess::path_visual_bounds(&computed, has_fill, stroke_params.as_ref());
...
mod tests {
    ...
    fn selection_bounds_of_translated_group_applies_transform_once() {
        ...
        if let NodeData::Path { ref mut style, .. } = path_node.data {
            style.fill = Some(locus_scene::Fill {
                paint: locus_scene::PaintRef::Solid(locus_geom::Color::from_srgb8(255, 0, 0, 255)),
