...
use locus_geom::{Affine, Color, Segment};
use locus_scene::{
...
fn write_node(
    ...
    match &node.data {
        ...
        NodeData::Group {
            is_defs: false,
            kind: locus_scene::GroupKind::Boolean { op, style },
        } => {
            ...
            let op_name = match op {
                locus_scene::BoolOp::Union => "union",
                locus_scene::BoolOp::Difference => "difference",
                locus_scene::BoolOp::Intersect => "intersect",
                locus_scene::BoolOp::Exclude => "exclude",
            };
...
fn write_stops(stops: &[locus_scene::ColorStop], indent: usize, buf: &mut String) {
    let pad = "  ".repeat(indent);
    for stop in stops {
...
fn fmt_path_data(path: &locus_geom::Path) -> String {
...
mod tests {
    ...
    fn export_simple_path() {
        ...
        let mut node = locus_scene::Node::path("triangle", path);
    ...
    fn export_gradient_defs() {
        use locus_scene::{ColorStop, Gradient, GradientKind, InterpolationSpace, SpreadMethod};
        ...
        let grad_node = locus_scene::Node {
            label: "my_grad".into(),
            transform: Affine::IDENTITY,
        ...
        let mut node = locus_scene::Node::path("rect", path);
        if let NodeData::Path { ref mut style, .. } = node.data {
            style.fill = Some(locus_scene::style::Fill {
    ...
    fn export_pattern_defs() {
        ...
        let content_group = locus_scene::Node::group("dots_content");
        ...
        let mut dot_node = locus_scene::Node::path("dot", dot_path);
        ...
        let paint_node = locus_scene::Node {
        ...
        let mut rect_node = locus_scene::Node::path("rect", rect_path);
