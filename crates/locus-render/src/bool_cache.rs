...
/// Caches `locus_bool::compute_boolean_group_path` results per boolean
...
impl BoolPathCache {
    ...
    pub fn get_or_compute(&mut self, scene: &Scene, group_id: NodeId) -> &Path {
        ...
        if stale {
            let path = locus_bool::compute_boolean_group_path(scene, group_id);
            self.entries.insert(
                group_id,
                CachedBoolPath {
                    subtree_rev: rev,
...
mod tests {
    ...
    use locus_scene::{BoolOp, GroupKind, Node, Scene, Style};
    ...
    fn cache_miss_when_descendant_transform_changes() {
        ...
        scene
            .set_transform(a, locus_geom::Affine::translate(100.0, 0.0))
