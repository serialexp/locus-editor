...
use locus_geom::{Affine, Color, Point, Segment};
use locus_scene::{
...
impl Renderer {
    ...
    pub fn prepare(
        ...
        let resolve_stroke = |s: &Stroke| -> StrokeParams {
            let cap = match s.style.cap {
                locus_scene::LineCap::Butt => LineCap::Butt,
                locus_scene::LineCap::Round => LineCap::Round,
                locus_scene::LineCap::Square => LineCap::Square,
            };
            let join = match s.style.join {
                locus_scene::LineJoin::Miter => LineJoin::Miter,
                locus_scene::LineJoin::Round => LineJoin::Round,
                locus_scene::LineJoin::Bevel => LineJoin::Bevel,
            };
        ...
        scene.walk_depth_first(root, Affine::IDENTITY, &mut |id, node, world_transform| {
            ...
            match node.data {
                NodeData::Group {
                    kind: locus_scene::GroupKind::Boolean { ref style, .. },
                    is_defs: false,
                } => {
    ...
    fn build_handles(
        ...
        if selection.mode == SelectionMode::Object {
            ...
            for id in selected_for_bbox {
                ...
                let bounds = match &node.data {
                    NodeData::Group {
                        kind: locus_scene::GroupKind::Boolean { .. },
                        ..
                    } => {
                    ...
                    NodeData::Group { .. } => {
                        ...
                        let mut b = locus_geom::Bounds::EMPTY;
            ...
            let mut combined_bounds = locus_geom::Bounds::EMPTY;
                ...
                if let Some(node) = scene.get(id) {
                    ...
                    let b = match &node.data {
                        ...
                        NodeData::Group { .. } => {
                            ...
                            // twice.
                            let parent_world = scene.parent_world_transform(id);
                            let mut gb = locus_geom::Bounds::EMPTY;
                            scene.walk_depth_first(id, parent_world, &mut |_cid, cnode, cworld| {
                                if !cnode.visible {
                                    return false;
                                }
                ...
                &mut |id, node, world_transform| {
                    ...
                    let bounds = if bounds.is_empty() {
                        if let NodeData::Text(text) = &node.data {
                            let shaped = locus_text::shape_text(
                                &text.content,
                                &text.font_family,
                                text.font_size,
                            );
                            let local = locus_geom::Bounds {
                                min: locus_geom::Point::new(0.0, -shaped.ascent),
                                max: locus_geom::Point::new(
        ...
        if selection.mode == SelectionMode::Node
            ...
            && let locus_scene::NodeData::Path { ref path, .. } = node.data
        ...
        if selection.mode == SelectionMode::Node {
            ...
            fn draw_handle_line(
                ...
                match mode {
                    locus_geom::VertexMode::Smooth | locus_geom::VertexMode::Symmetric => {
                        push_line(verts, idxs, anchor, ctrl, thickness, color);
                    }
                    locus_geom::VertexMode::Corner => {
                        push_dashed_line(
                    ...
                    for subpath in &path.subpaths {
                        ...
                        for (seg_idx, seg) in subpath.segments.iter().enumerate() {
                            ...
                            let from_mode = subpath
                                ...
                                .get(seg_idx)
                                .copied()
                                .unwrap_or(locus_geom::VertexMode::Corner);
        ...
        // `for_each_handle_of_gradient` helper resolves this for us.
        {
            ...
            // scene directly here (rather than re-using a locus-tools
