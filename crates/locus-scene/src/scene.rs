...
impl Scene {
    ...
    /// `locus_bool::scene_content_bounds`, which short-circuits at
    /// Boolean groups and returns their tight baked-path bounds instead
    /// of the looser union-of-operands bounds.
    pub fn content_bounds(&self) -> locus_geom::Bounds {
        let mut bounds = locus_geom::Bounds::EMPTY;
...
mod tests {
    use super::*;
    use locus_geom::{Point, Segment, SubPath, VertexMode};
