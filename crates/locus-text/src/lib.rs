...
//! a `locus_geom::Path` ready for tessellation.
//!
//! ## Font resolution
//!
...
use locus_geom::{Bounds, Path, Point, Segment, SubPath, VertexMode};

/// The embedded fallback font (Liberation Sans Regular, SIL OFL license).
/// Public so other crates (e.g. locus-svg) can load it into their own font databases.
