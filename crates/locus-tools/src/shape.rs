...
use locus_geom::{Color, Path, Point, Segment, SubPath, Vec2, VertexMode};
...
impl ShapeDrawState {
    ...
    pub fn on_press(&mut self, scene: &mut Scene, canvas_pos: [f64; 2], tool: ToolType) {
        ...
        let mut node = locus_scene::Node::path(tool.name(), path);
        ...
        if let NodeData::Path { ref mut style, .. } = node.data {
            style.fill = Some(locus_scene::style::Fill {
                paint: locus_scene::PaintRef::Solid(Color::from_srgb8(70, 130, 180, 255)),
